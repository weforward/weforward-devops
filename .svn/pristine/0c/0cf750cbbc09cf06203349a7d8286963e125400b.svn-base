...
public class TracerMethods {
	...
	public ResultPage<TracerTreeView> search(TracerParam params) throws ApiException {
		...
		if (!StringUtil.isEmpty(id)) {
			TracerSpanTree tree = m_MetricsService.getTracer(id);
			if (null != tree) {
			...
			rp = m_MetricsService.searchTracer(params.getBegin(), params.getEnd(), params.getServiceName(),
	...
	public TracerTreeDetailView get(IdBean params) throws ApiException {
		...
		return TracerTreeDetailView.valueOf(m_MetricsService.getTracer(params.getId()));
	}
}
